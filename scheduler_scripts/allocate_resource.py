...
def get_available_gpu():
    ...
    try:
        result = subprocess.run(
            ["nvidia-smi", "--query-gpu=gpu_name,memory.used", "--format=csv,nounits,noheader"],
            capture_output=True,
            text=True,
        )
...
def get_mac_address():
    ...
    try:
        ...
        mac = ":".join([f"{(getnode() >> elements) & 0xFF:02x}" for elements in range(0, 8 * 6, 8)][::-1])
...
def allocate_resource(allocated_by):
    ...
    try:
        response = requests.post(
            ...
            json={"name": name, "gpu_name": gpu_name, "allocated_by": allocated_by, "mac_address": mac_address},
        )
...
def send_resource_heartbeat():
    ...
    try:
        ...
        response = requests.put(f"https://{endpoint}/api/resource/heartbeat", json={"mac_address": mac_address})
