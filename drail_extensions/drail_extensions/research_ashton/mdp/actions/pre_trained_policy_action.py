...
class PreTrainedPolicyAction(ActionTerm):
    ...
    The actions correspond to the commands for the pre-trained policy.

    """

    cfg: actions_cfg.PreTrainedPolicyActionCfg
    """The configuration of the action term."""

    def __init__(self, cfg: actions_cfg.PreTrainedPolicyActionCfg, env: ManagerBasedRLEnv) -> None:
        ...
        self._stand_bit = torch.zeros(self.num_envs, device=self.device)
        ...
        self._whole_body_rest_cmds = torch.zeros_like(self._actions)
        self._whole_body_rest_cmds[:, 0] = 0.35 # height
        self._whole_body_rest_cmds[:, 1] = 0.0 # roll
        self._whole_body_rest_cmds[:, 2] = 0.0 # pitch
    ...
    def processed_actions(self) -> torch.Tensor:
        return torch.cat([self._actions, self._stand_bit.unsqueeze(1), self._whole_body_rest_cmds], dim=1)
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        # set the stand bit the last action based on the other actions
        vel_norm = torch.linalg.norm(self._actions[:, :3], dim=1)
        self._stand_bit = (vel_norm < self.cfg.action_threshold).float()

        # zero out the actions if the stand bit is set
        self._actions[vel_norm < self.cfg.action_threshold, :3] = 0.0
