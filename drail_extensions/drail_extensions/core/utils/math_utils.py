...
def euler_xyz_from_quat(quat: torch.Tensor) -> torch.Tensor:
    return math_utils.wrap_to_pi(
        torch.stack(math_utils.euler_xyz_from_quat(quat.view(-1, 4)), dim=-1).view(*quat.shape[:-1], 3)
    )
