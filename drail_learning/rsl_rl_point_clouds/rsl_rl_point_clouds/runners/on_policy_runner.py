...
class OnPolicyRunner(OnPolicyRunnerBase):
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        self.num_point_cloud_obs = extras["observations"]["point_cloud"].shape[1:]
        actor_critic_class = eval(self.policy_cfg.pop("class_name"))  # ActorCritic
        self.actor_critic: ActorCritic = actor_critic_class(
            self.num_obs, self.num_critic_obs, self.num_point_cloud_obs, self.env.num_actions, **self.policy_cfg
        ).to(self.device)
        ...
        self.git_status_repos = [rsl_rl.__file__, rsl_rl_point_clouds.__file__]
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.no_grad():
                for _ in range(self.num_steps_per_env):
                    ...
                    point_cloud_obs = extras["observations"]["point_cloud"]
                    actions = self.alg.act(obs, critic_obs, point_cloud_obs)
                    # Step environment
                ...
                start = stop
                point_cloud_obs = extras["observations"]["point_cloud"]
                self.alg.compute_returns(critic_obs, point_cloud_obs)
