...
class RolloutStorage(RolloutStorageBase):
    class Transition(RolloutStorageBase.Transition):
        def __init__(self):
            super().__init__()
            self.point_cloud_observations = None
    ...
    def __init__(
        ...
        self.point_cloud_obs_shape = point_cloud_obs_shape
    ...
    def setup_transition_buffer(self):
        ...
        self.experience_buffer["point_cloud_observations"] = torch.zeros(
            self.num_transitions_per_env, self.num_envs, *self.point_cloud_obs_shape, device=self.device
        )
        self.observation_keys.append("point_cloud_observations")

    def add_transitions(self, transition: Transition):
        if self.step >= self.num_transitions_per_env:
            raise OverflowError("Rollout buffer overflow! You should call clear() before adding new transitions.")

        self.experience_buffer["point_cloud_observations"][self.step].copy_(transition.point_cloud_observations)
        super().add_transitions(transition)

    def process_raw_mini_batch(self, raw_mini_batch):
        processed_mini_batch = super().process_raw_mini_batch(raw_mini_batch)
        processed_mini_batch["point_cloud_obs_batch"] = raw_mini_batch["point_cloud_observations"]
