...
import rsl_rl_ashton
from rsl_rl_ashton.algorithms import *
...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        self.num_critic_obs = extras["observations"]["critic"].shape[1] if "critic" in extras["observations"] else self.num_obs

        actor_critic_class = eval(self.policy_cfg.pop("class_name"))  # ActorCritic
        self.actor_critic: ActorCritic | ActorCriticRecurrent = actor_critic_class(
            self.num_obs, self.num_critic_obs, self.env.num_actions, **self.policy_cfg
        ).to(self.device)
        ...
        self.git_status_repos = [rsl_rl_ashton.__file__]
    ...
    def init_logger(self):
        ...
        if self.log_dir is not None and self.writer is None:
            ...
            if self.logger_type == "neptune":
                from rsl_rl_ashton.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.cfg)
                self.writer.log_config(self.env.cfg, self.cfg, self.alg_cfg, self.policy_cfg)
            elif self.logger_type == "wandb":
                from rsl_rl_ashton.utils.wandb_utils import WandbSummaryWriter
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):
        ...
        # Book keeping
        ep_infos = []
        # NOTE: Could increase the size of the buffer
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        # -- Losses
        for loss_key, loss_value in locs["loss_dict"].items():
            self.writer.add_scalar(f"Loss/{loss_key}", loss_value, locs["it"])
        ...
        self.writer.add_scalar("Iterations", locs["it"], locs["it"])
        ...
        if len(locs["rewbuffer"]) > 0:
            log_string = (
                ...
                f"""{'KL divergence:':>{pad}} {locs['loss_dict']['mean_kl']:.4f}\n"""
            )

            # -- For symmetry
            if self.alg.symmetry:
                log_string += f"""{'Symmetry loss:':>{pad}} {locs['loss_dict']['mean_symmetry_loss']:.4f}\n"""
    ...
    def load(self, path: str, load_optimizer: bool = True, itr: int = None):
        ...
        self.current_learning_iteration =  itr if itr is not None else loaded_dict["iter"]
