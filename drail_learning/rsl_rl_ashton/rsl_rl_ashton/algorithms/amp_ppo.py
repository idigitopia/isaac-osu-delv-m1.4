...
from rsl_rl_ashton.storage import AMPRolloutStorage
from rsl_rl_ashton.algorithms import PPO
...
class AMPPPO(PPO):
    ...
    def act(self, obs, critic_obs, amp_obs):
        ...
        return super(AMPPPO, self).act(obs, critic_obs)
    ...
    def _train_discriminator(self):
        ...
        if self.actor_critic.is_recurrent:
            generator = self.storage.reccurent_mini_batch_generator(num_mini_batches=num_mini_batches, num_epochs=self.num_learning_epochs)
        else:
            generator = self.storage.mini_batch_generator(num_mini_batches=num_mini_batches, num_epochs=self.num_learning_epochs)
    ...
    def update(self):
        ...
        ppo_losses = super(AMPPPO, self).update()
        # Merge the two dictionaries
        loss_dict = {**ppo_losses, **disc_losses}
        return loss_dict
