...
from rsl_rl_ashton.modules import ActorCritic
from rsl_rl_ashton.modules.rnd import RandomNetworkDistillation
from rsl_rl_ashton.storage import RolloutStorage
from rsl_rl_ashton.utils import string_to_callable
...
class PPO:
    ...
    def __init__(
        ...
        if self.lr_scheduler:
            self.optim_lr_scheduler = optim.lr_scheduler.StepLR(
    ...
    def update(self):  # noqa: C901
        ...
        for processed_mini_batch in generator:
            ...
            hid_states_batch = processed_mini_batch.get("hid_a", None), processed_mini_batch.get("hid_c", None)
            masks_batch = processed_mini_batch.get("obs_mask_batch", None)
            rnd_state_batch = processed_mini_batch.get("rnd_state_batch", None)
            ...
            ## Adapt learning rate based on fixed schedule
            if self.lr_scheduler:
        ...
        mean_kl = sum(kl_means) / num_updates
        # -- For RND
        if mean_rnd_loss is not None:
