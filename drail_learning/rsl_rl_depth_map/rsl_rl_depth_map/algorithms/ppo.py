...
from rsl_rl_depth_map.modules import ActorCritic
from rsl_rl_depth_map.storage import RolloutStorage
...
class PPO(PPOBase):
    ...
    def init_storage(
        self, num_envs, num_transitions_per_env, actor_obs_shape, critic_obs_shape, depth_map_obs_shape, action_shape
    ):
    ...
    def act(self, obs, critic_obs, depth_map_obs):
        if self.actor_critic.is_recurrent:
            self.transition.hidden_states = self.actor_critic.get_hidden_states()
        # Compute the actions and values
        self.transition.actions = self.actor_critic.act(obs, depth_map_obs).detach()
        self.transition.values = self.actor_critic.evaluate(critic_obs, depth_map_obs).detach()
        ...
        self.transition.depth_map_observations = depth_map_obs
        return self.transition.actions

    def compute_returns(self, last_critic_obs, last_depth_map_obs):
        # compute value for the last step
        last_values = self.actor_critic.evaluate(last_critic_obs, last_depth_map_obs).detach()
    ...
    def update(self):  # noqa: C901
        ...
        for processed_mini_batch in generator:
            ...
            depth_map_obs_batch = processed_mini_batch["depth_map_obs_batch"]
            ...
            if self.symmetry and self.symmetry["use_data_augmentation"]:
                ...
                obs_batch, depth_map_obs_batch, actions_batch = data_augmentation_func(
                    obs=obs_batch,
                    depth_map_obs=depth_map_obs_batch,
            ...
            # -- actor
            self.actor_critic.act(obs_batch, depth_map_obs_batch, masks=masks_batch, hidden_states=hid_states_batch[0])
            actions_log_prob_batch = self.actor_critic.get_actions_log_prob(actions_batch)
            # -- critic
            value_batch = self.actor_critic.evaluate(
                critic_obs_batch, depth_map_obs_batch, masks=masks_batch, hidden_states=hid_states_batch[1]
            )
            # -- entropy
